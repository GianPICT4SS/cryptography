...
with socket.create_connection((hostname, portnumber)) as sock:
    with context.wrap_socket(sock, server_hostname=hostname) as ssock:
        ...
        if isinstance(public_key, rsa.RSAPublicKey):
            print('  RSA public key:\n', public_key.public_bytes(serialization.Encoding.PEM, serialization.PublicFormat.SubjectPublicKeyInfo).decode('utf-8'), sep='')
        ...
        if isinstance(hash_algorithm, hashes.SHA256):
            ...
            print('  WARNING: SHA1 has been deprecated, this certificate should not be trusted!')
        else:
        ...
        if hostname == '127.0.0.1' or hostname == 'localhost':
            ...
            # the ssl module do not have a function to download the current issuer certificate. we will use certificates downloaded with openssl on 29/05/2020. those may become invalid in the future.
            if hostname == 'www.google.com':
                with open("google_cert.pem", "rb") as bin_file:
                    pem_data = bin_file.read()
            elif hostname == 'www.polito.it':
                with open("polito_cert.pem", "rb") as bin_file:
                    pem_data = bin_file.read()
            else:
                exit()
